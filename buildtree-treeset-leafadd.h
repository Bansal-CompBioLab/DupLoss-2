...
#ifndef BUILDTREE_TREESET_LEAFADD_H
#define BUILDTREE_TREESET_LEAFADD_H
...
// ------------------------------------------------------------------------------------------------------------------
// Data structures for computing Losses
class GeneLoss {
public:
	int subtreeSize, lossCounter1, lossCounter2, lossCounter3, lossCounter4, lossCounter5, lossCounter6, lossScore, LossScoreDiff, nodeDepth; 
...
ostream & operator << (ostream & os, GeneLoss & m) {
	...
	os << "Losses:" << m.subtreeSize << " " << m.isRelevant << " " << m.lossCounter1 << " " << " " << m.lossCounter2 << " " << m.lossCounter3 << " " << m.lossCounter4 << " " << m.lossCounter5 << " " << m.lossCounter6 << " : "<< m.lossScore;
	return os;
}
...
public:
	int idx;
	int constraint;
	SpeciesNode *LossParent, *LossChild1, *LossChild2 ;

	SpeciesNode(SpeciesNode  *parent = NULL) : TreeNodeRooted<SpeciesNode>(parent) {
		constraint = -1;
	}

	SpeciesNode *&getLossSibling()
	{
	#ifdef DEBUG
		if (LossParent == NULL) EXCEPTION("getSibling: node has no sibling");
		#endif
		return LossParent->LossChild1 == this ? LossParent->LossChild2 : LossParent->LossChild1;
	}
...
public:
	...
	void namednode2newick(ostream &os, NamedSpeciesNode &node) {
		...
		if (node.parent() != NULL) {
			if (node.constraint != node.parent()->constraint) os << "[&CONSTRAINT]";
		}
	}
...
// 	// outputs the tree in newick format with constraints
// 	public: void tree2newick(ostream &os) {
// 		subtree2newickDFS(os, root, -1);
// 		os << ';';
// 	}
// 	protected: void subtree2newickDFS(ostream &os, NODE *&node, int lastconstraint) {
// 		if (node == NULL) return;
// 		// if leaf node then output the node
// 		if (node->isLeaf()) {
// 			EXCEPTIONDATATYPE("tree2newickDFS", *node, NAMEDNODE);
// 			os << name2newick(((NAMEDNODE *)node)->getName());
// 			if (node->constraint != lastconstraint) {
// 				WARNING("leaf is constraint" << endl);
// 			}
// 		} else {
...
// 			subtree2newickDFS(os, node->child(0), node->constraint);
// 			if (node->child(1)!=NULL) os << ',';
// 			subtree2newickDFS(os, node->child(1), node->constraint);
// 			// close the subtree
// 			if ((node->child(0)!=NULL) || (node->child(1)!=NULL)) os << ')';
// 			// output name
// 			if (typeid(*node)==typeid(NAMEDNODE)) os << name2newick(((NAMEDNODE *)node)->getName());
// 		}
// 		if (node->constraint != lastconstraint) {
// 			os << "[&CONSTRAINT]";
// 		}
...
//					msgout << "input gene tree of " << tree->leafnodes.size() << " taxa" << endl;
				} else {
					delete tree;
					break;
				}
...
//					msgout << "Unrooted input gene tree of " << tree->leafnodes.size() << " taxa" << endl;
	...
	void readConstraints(Input &input) {
		char c;
		for (;;) {
			if (!input.nextAnyChar(c)) break;
			input.pushBack(c);

			// read set of species that form one constrain from input
			vector<string> names;
			for (;;) {
				string name = input.getName();
				if (name.size() == 0) EXCEPTION("expected species name in constraints input " << input.getPos());

				// check if name exist
				map<string,int>::iterator n = NodeID::namelist.find(name);
				if (n == NodeID::namelist.end()) { // name doesn't exist
					WARNING("species '" << name << "' listed in constraints input was not found in taxa set - species ignored");
				} else {
					names.push_back(name);
				}


				if (!input.nextAnyChar(c)) EXCEPTION("unexpected end constraints input " << input.getPos());
				if (c == ';') break;
				if (c != ',') EXCEPTION("expected ',' in constraints input " << input.getPos());
			}
			if (names.size() < 2) {
				WARNING("constraint has to contain at least 2 species - constraint ignored");
			} else {
				constraints.push_back(names);
				msgout << "Constraint of " << names.size() << " taxa" << endl;
			}
		}
		...
		if (constraints.size() == 0) {
			...
			msgout << constraints.size() << " constraints total" << endl;
		}
	}
	...
	void createLeafMapping() {
		...
		// create the leaf node mapping for all gene trees
...
/*
	...
	SpeciesNode* &getLCA(GeneNodeRooted* &node, GeneNodeRooted* &node2, VAL E[], INT R[], struct rmqinfo *&ri) {
		...
		if (gn[1] == NULL) {
			...
			SpeciesNode *&p = (u==NULL)?v:(v==NULL)?u:speciestree->getLCA(u, v);
			node->setMapping(p);
		}
	...
	SpeciesNode* &get2ndLCA(GeneNode* &node, SpeciesNode &subtreeroot, VAL E[], INT R[], struct rmqinfo *&ri) {
		...
		#endif
			//cout<<"node "<<node<<endl;
			//cout<<"tree "<<*genetree_rooted[0]<<endl;
		...
		if (mapping1 != speciestree->root) {
			...
			if ((u == NULL) && (v == NULL)) mapping2 = NULL;
			else
	...
	template<class GeneTree>
	inline int getScore(GeneTree &genetree) {
		int score = 0;
		getScoreDFS(score, genetree.root);


//if(speciestree->leafnodes.size() >=8)
//{
//cout << " score : "<< score << endl;
//cout << "genetree: " << genetree << endl;
//cout << "tree in get score: " << endl << *speciestree;
//}
//cout << genetree << endl<< *speciestree << endl << score << endl;
//exit(0);
		return score;
	}
	template<class GeneNode>
	inline void getScoreDFS(int &score, GeneNode *&genenode) {
		if ((!genenode->isLeaf()) &&  (genenode->getMapping() !=NULL)) {
			// child left
			...
			SpeciesNode *&nodemap = genenode->getMapping();
				...
				int tempvar =0, tempvar2=0;

				if((nodemap == child1->getMapping()) && (child2->getMapping() != NULL))
				{
//					if(!child1->isLeaf())
//					{
//						if ((child1->child(0)->getMapping() !=NULL) && (child1->child(1)->getMapping() !=NULL))
//							tempvar =1;
//
//					}
//					else
				...
				if((nodemap == child2->getMapping()) && (child1->getMapping() != NULL ))
				{
//					if(!child2->isLeaf())
//					{
//						if ((child2->child(0)->getMapping() !=NULL) && (child2->child(1)->getMapping() !=NULL))
//							tempvar =1;
//
//					}
//					else
